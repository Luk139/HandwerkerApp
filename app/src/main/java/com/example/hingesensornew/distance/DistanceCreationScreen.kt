...
fun DistanceCreationScreen(
    ...
    Column(modifier = Modifier.padding(8.dp)) {
        ...
        Row(
            ...
            Button(
                onClick = {
                    val distanceItemEntity = DistanceItemEntity(
                        title = nameState.value.text,
                        distance = distance,
                        measurementOwnerId = measurementId
                    )
                    Log.d("NEW Distance","Added to $measurementId")
                    distanceCreationScreenViewModel.insertDistanceEntity(distanceItemEntity)
                    navController.navigate(Routes.DISTANCE.value)
                },
                modifier = modifier
                    .padding(8.dp)
                    .width(150.dp)
...
@Preview
@Composable
private fun DistanceCreationScreenPreview() {
    MaterialTheme {
